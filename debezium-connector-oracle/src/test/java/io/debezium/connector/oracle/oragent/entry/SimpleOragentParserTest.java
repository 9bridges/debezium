 ...
 */
package io.debezium.connector.oracle.oragent.entry;

import java.util.Objects;
import java.util.concurrent.BlockingQueue;
import java.util.concurrent.LinkedBlockingQueue;

import net.tbsoft.oragentclient.client.entry.*;
import org.junit.Before;
import org.junit.Test;

public class SimpleOragentParserTest {
    // test table is create table oragent.test (tint int,tchar char(10),tvarchar2 varchar2(20));

    // insert into oragent.test values(123,'charval','varcharval');
    ...
    // update oragent.test set tint=456,tchar='newval',tvarchar2='newvarcharval' where tint=123;
    ...
    // delete from oragent.test where tint=456;
    ...
    /* insert into oragent.test values(1,'col1','col2');
       insert into oragent.test values(2,'col1','col2');
       insert into oragent.test values(3,'col1','col2');
       insert into oragent.test values(4,'col1','col2');
       insert into oragent.test values(5,'col1','col2');
     */
    ...
    SimpleOragentParser simpleOragentParser = new SimpleOragentParser();
    BlockingQueue<OragentEntry> recordQueue = new LinkedBlockingQueue<>();
    ...
    public void UpdateTest() {
        simpleOragentParser.parser(UPDATE, recordQueue);
        assert !recordQueue.isEmpty();
        OragentEntry oragentEntry = recordQueue.poll();
        assert oragentEntry.getEventType().equals(OpCode.UPDATE);
        OragentDmlUrp oragentDmlUrp = (OragentDmlUrp) oragentEntry;
        assert oragentDmlUrp.getObjectOwner().equals(objectOwner);
        assert oragentDmlUrp.getObjectName().equals(objectName);
        String[] parseOldColumnNames = oragentDmlUrp.getOldColumnNames();
        Object[] parseOldValues = oragentDmlUrp.getOldValues();
        String[] parseNewColumnNames = oragentDmlUrp.getNewColumnNames();
        Object[] parseNewValues = oragentDmlUrp.getNewValues();
    ...
    public void InsertTest() {
        simpleOragentParser.parser(INSERT, recordQueue);
        assert !recordQueue.isEmpty();
        OragentEntry oragentEntry = recordQueue.poll();
        assert oragentEntry.getEventType().equals(OpCode.INSERT);
        OragentDmlIrp oragentDmlIrp = (OragentDmlIrp) oragentEntry;
        assert oragentDmlIrp.getObjectOwner().equals(objectOwner);
        assert oragentDmlIrp.getObjectName().equals(objectName);
        String[] parseColumnNames = oragentDmlIrp.getNewColumnNames();
        Object[] parseValues = oragentDmlIrp.getNewValues();
    ...
    public void DeleteTest() {
        simpleOragentParser.parser(DELETE, recordQueue);
        assert !recordQueue.isEmpty();
        OragentEntry oragentEntry = recordQueue.poll();
        assert oragentEntry.getEventType().equals(OpCode.DELETE);
        OragentDmlDrp oragentDmlDrp = (OragentDmlDrp) oragentEntry;
        assert oragentDmlDrp.getObjectOwner().equals(objectOwner);
        assert oragentDmlDrp.getObjectName().equals(objectName);
        String[] parseColumnNames = oragentDmlDrp.getOldColumnNames();
        Object[] parseValues = oragentDmlDrp.getOldValues();
        assert parseColumnNames.length == COLUMN_COUNT;
        for (int i = 0; i < COLUMN_COUNT; i++) {
            assert parseColumnNames[i].equals(columnNames[i]);
    ...
    public void MulitInsertTest() {
        simpleOragentParser.parser(MULIT_INSERT, recordQueue);
        assert recordQueue.size() == 6;
        for (int row = 0; row < 5; row++) {
            OragentEntry oragentEntry = recordQueue.poll();
            assert Objects.requireNonNull(oragentEntry).getEventType().equals(OpCode.INSERT);
            OragentDmlIrp oragentDmlIrp = (OragentDmlIrp) oragentEntry;
            assert oragentDmlIrp.getObjectOwner().equals(objectOwner);
            assert oragentDmlIrp.getObjectName().equals(objectName);
            String[] parseColumnNames = oragentDmlIrp.getNewColumnNames();
