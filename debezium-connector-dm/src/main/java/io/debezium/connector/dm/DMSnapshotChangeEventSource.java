...
/**
 * A {@link StreamingChangeEventSource} for DM.
 *
 * @author Gunnar Morling
 */
public class DMSnapshotChangeEventSource extends RelationalSnapshotChangeEventSource<DMOffsetContext> {

    private static final Logger LOGGER = LoggerFactory.getLogger(DMSnapshotChangeEventSource.class);

    private final DMConnectorConfig connectorConfig;
    private final DMConnection jdbcConnection;

    public DMSnapshotChangeEventSource(DMConnectorConfig connectorConfig, DMConnection jdbcConnection,
                                       DMDatabaseSchema schema, EventDispatcher<TableId> dispatcher, Clock clock,
    ...
    @Override
    protected SnapshottingTask getSnapshottingTask(DMOffsetContext previousOffset) {
    ...
    @Override
    protected SnapshotContext<DMOffsetContext> prepare(ChangeEventSourceContext context) throws Exception {
        if (connectorConfig.getPdbName() != null) {
            jdbcConnection.setSessionToPdb(connectorConfig.getPdbName());
        }

        return new DMSnapshotContext(connectorConfig.getCatalogName());
    }

    @Override
    protected Set<TableId> getAllTableIds(RelationalSnapshotContext<DMOffsetContext> ctx) throws Exception {
    ...
    @Override
    protected void lockTablesForSchemaSnapshot(ChangeEventSourceContext sourceContext, RelationalSnapshotContext<DMOffsetContext> snapshotContext)
            throws SQLException, InterruptedException {
        if (connectorConfig.getSnapshotLockingMode().usesLocking()) {
            ((DMSnapshotContext) snapshotContext).preSchemaSnapshotSavepoint = jdbcConnection.connection().setSavepoint("dbz_schema_snapshot");
    ...
    @Override
    protected void releaseSchemaSnapshotLocks(RelationalSnapshotContext<DMOffsetContext> snapshotContext) throws SQLException {
        if (connectorConfig.getSnapshotLockingMode().usesLocking()) {
            jdbcConnection.connection().rollback(((DMSnapshotContext) snapshotContext).preSchemaSnapshotSavepoint);
        }
    }

    @Override
    protected void determineSnapshotOffset(RelationalSnapshotContext<DMOffsetContext> ctx,
                                           DMOffsetContext previousOffset)
            throws Exception {
        ...
        ctx.offset = DMOffsetContext.create()
    ...
    private Scn getCurrentScn(SnapshotContext<DMOffsetContext> ctx) throws SQLException {
     ...
     */
    private Optional<Scn> getLatestTableDdlScn(RelationalSnapshotContext<DMOffsetContext> ctx) throws SQLException {
    ...
    @Override
    protected void readTableStructure(ChangeEventSourceContext sourceContext, RelationalSnapshotContext<DMOffsetContext> snapshotContext,
        ...
        for (String schema : schemas) {
            ...
            // if (connectorConfig.getAdapter().equals(DMConnectorConfig.ConnectorAdapter.LOG_MINER)) {
    ...
    @Override
    protected String enhanceOverriddenSelect(RelationalSnapshotContext<DMOffsetContext> snapshotContext, String overriddenSelect, TableId tableId) {
    ...
    @Override
    protected SchemaChangeEvent getCreateTableEvent(RelationalSnapshotContext<DMOffsetContext> snapshotContext, Table table) throws SQLException {
    ...
    @Override
    protected void complete(SnapshotContext<DMOffsetContext> snapshotContext) {
        if (connectorConfig.getPdbName() != null) {
     ...
     */
    private static class DMSnapshotContext extends RelationalSnapshotContext<DMOffsetContext> {

        private Savepoint preSchemaSnapshotSavepoint;

        public DMSnapshotContext(String catalogName) throws SQLException {
            super(catalogName);
        }
    }
}
