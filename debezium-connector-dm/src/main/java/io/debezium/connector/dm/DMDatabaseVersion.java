...
/**
 * Represents the DM database version.
 *
 * @author Chris Cranford
 */
public class DMDatabaseVersion {
    ...
    private DMDatabaseVersion(int major, int maintenance, int appServer, int component, int platform, String banner) {
        this.major = major;
    ...
    /**
     * Parse the DM database version banner.
     *
     * @param banner the banner text
     * @return the parsed DMDatabaseVersion.
     * @throws RuntimeException if the version banner string cannot be parsed
     */
    public static DMDatabaseVersion parse(String banner) {
        Matcher matcher = VERSION_18_1_PATTERN.matcher(banner);
        if (!matcher.matches()) {
            matcher = VERSION_PATTERN.matcher(banner);
            if (!matcher.matches()) {
                throw new RuntimeException("Failed to resolve DM database version: '" + banner + "'");
            }
        }
        ...
        return new DMDatabaseVersion(major, maintenance, app, component, platform, banner);
    }
}
