...
import io.debezium.connector.dm.logminer.HistoryRecorder;
import io.debezium.connector.dm.logminer.NeverHistoryRecorder;
import io.debezium.connector.dm.logminer.SqlUtils;
...
/**
 * Connector configuration for DM.
 *
 * @author Gunnar Morling
 */
public class DMConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
    ...
    public static final Field HOSTNAME = RelationalDatabaseConnectorConfig.HOSTNAME
            .withNoValidation()
            .withValidation(DMConnectorConfig::requiredWhenNoUrl);
    ...
    public static final Field XSTREAM_SERVER_NAME = Field.create(DATABASE_CONFIG_PREFIX + "out.server.name")
            ...
            .withWidth(Width.MEDIUM)
            .withImportance(Importance.HIGH)
            .withValidation(DMConnectorConfig::validateOutServerName)
    ...
    public static final Field ORACLE_VERSION = Field.createInternal("database.dm.version")
            .withDisplayName("DM version, 11 or 12+")
            .withType(Type.STRING)
            .withImportance(Importance.LOW)
            .withDescription("Deprecated: For default DM 12+, use default pos_version value v2, for DM 11, use pos_version value v1.");
    ...
    public static final Field CONNECTOR_ADAPTER = Field.create(DATABASE_CONFIG_PREFIX + "connection.adapter")
            ...
            .withDescription("The adapter to use when capturing changes from the database. "
                    + "Options include: "
                    + "'logminer': (the default) to capture changes using native DM LogMiner; "
                    + "'xstream' to capture changes using DM XStreams");
    ...
    // this option could be true up to DM 18c version. Starting from DM 19c this option cannot be true todo should we do it?
    ...
    public static final Field RAC_NODES = Field.create("rac.nodes")
            .withDisplayName("DM RAC nodes")
            .withType(Type.STRING)
            .withWidth(Width.SHORT)
            .withImportance(Importance.HIGH)
            .withValidation(DMConnectorConfig::validateRacNodes)
    ...
    public static final Field URL = Field.create(DATABASE_CONFIG_PREFIX + "url")
            ...
            .withImportance(Importance.HIGH)
            .withValidation(DMConnectorConfig::requiredWhenNoHostname)
    ...
    public static final Field ORAGENT_SERVER_PORT = Field.create("oragent.server.port")
            .withDisplayName("oragent server port")
            .withType(Type.LONG)
            .withWidth(Width.SHORT)
            .withImportance(Importance.LOW)
    ...
    public DMConnectorConfig(Configuration config) {
        super(DMConnector.class, config, config.getString(SERVER_NAME), new SystemTablesPredicate(config), x -> x.schema() + "." + x.table(), true,
        ...
        this.dmVersion = config.getString(ORACLE_VERSION);
    ...
    public int getOragentServerPort() {
        return oragentServerPort;
    }
    ...
    public enum ConnectorAdapter implements EnumeratedValue {
        ...
        XSTREAM("XStream") {
            ...
            public String getConnectionUrl() {
                return "jdbc:dm:oci:@${" + JdbcConfiguration.HOSTNAME + "}:${" + JdbcConfiguration.PORT + "}/${" + JdbcConfiguration.DATABASE + "}";
            }

            @Override
            public StreamingAdapter getInstance(DMConnectorConfig connectorConfig) {
                return Instantiator.getInstanceWithProvidedConstructorType(
                        "io.debezium.connector.dm.xstream.XStreamAdapter",
                        StreamingAdapter.class::getClassLoader,
                        DMConnectorConfig.class,
                        connectorConfig);
            }
        },
        ORAGENT("oragent") {
            ...
            public StreamingAdapter getInstance(DMConnectorConfig connectorConfig) {
                return Instantiator.getInstanceWithProvidedConstructorType(
                        "io.debezium.connector.dm.oragent.OragentAdapter",
                        StreamingAdapter.class::getClassLoader,
                        DMConnectorConfig.class,
                        connectorConfig);
            }
        ...
        LOG_MINER("LogMiner") {
            @Override
            public String getConnectionUrl() {
                // return "jdbc:dm:thin:@${" + JdbcConfiguration.HOSTNAME + "}:${" + JdbcConfiguration.PORT + "}/${" + JdbcConfiguration.DATABASE + "}";
            ...
            public StreamingAdapter getInstance(DMConnectorConfig connectorConfig) {
                return Instantiator.getInstanceWithProvidedConstructorType(
                        "io.debezium.connector.dm.logminer.LogMinerAdapter",
                        StreamingAdapter.class::getClassLoader,
                        DMConnectorConfig.class,
                        connectorConfig);
            }
        ...
        public abstract StreamingAdapter getInstance(DMConnectorConfig connectorConfig);
    ...
    /**
     * A {@link TableFilter} that excludes all DM system tables.
     *
     * @author Gunnar Morling
     */
    ...
    protected SourceInfoStructMaker<? extends AbstractSourceInfo> getSourceInfoStructMaker(Version version) {
        return new DMSourceInfoStructMaker(Module.name(), Module.version(), this);
    }
    ...
    /**
     * @return whether DM is using RAC
     */
    ...
    /**
     * @return set of node hosts or ip addresses used in DM RAC
     */
