...
public class LogMinerQueryBuilder {
     ...
     * @return the SQL string to be used to fetch changes from DM LogMiner
     */
    public static String build(DMConnectorConfig connectorConfig, DMDatabaseSchema schema, String userName) {
        ...
        if (!DMConnectorConfig.EXCLUDED_SCHEMAS.isEmpty()) {
            query.append("AND SEG_OWNER NOT IN (");
            for (Iterator<String> i = DMConnectorConfig.EXCLUDED_SCHEMAS.iterator(); i.hasNext();) {
     ...
     */
    private static String buildSchemaPredicate(DMConnectorConfig connectorConfig) {
     ...
     */
    private static String buildTablePredicate(DMConnectorConfig connectorConfig) {
        StringBuilder predicate = new StringBuilder();
    ...
    /**
     * Takes a list of reg-ex patterns and builds an DM-specific predicate using {@code REGEXP_LIKE}
    ...
    private static String listOfPatternsToSql(List<Pattern> patterns, String columnName, boolean inclusion) {
        ...
        for (Iterator<Pattern> i = patterns.iterator(); i.hasNext();) {
            ...
            // NOTE: The REGEXP_LIKE operator was added in DM 10g (10.1.0.0.0)
    ...
    /**
     * The {@code REGEXP_LIKE} DM operator acts identical to the {@code LIKE} operator. Internally,
