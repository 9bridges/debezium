...
public class DMConnector extends RelationalBaseSourceConnector {
    ...
    protected void validateConnection(Map<String, ConfigValue> configValues, Configuration config) {
        ...
        try (DMConnection connection = new DMConnection(connectorConfig.getJdbcConfig(), () -> getClass().getClassLoader())) {
            LOGGER.debug("Successfully tested connection for {} with user '{}'", DMConnection.connectionString(config), connection.username());
        }
    ...
    protected Map<String, ConfigValue> validateAllFields(Configuration config) {
        return config.validate(DMConnectorConfig.ALL_FIELDS);
    }
}
